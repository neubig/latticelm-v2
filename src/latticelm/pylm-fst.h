...
public:
  PylmFst(Pylm & pylm) : pylm_(&pylm), 
                               properties_(fst::kILabelSorted | fst::kOLabelSorted | fst::kExpanded | fst::kAcceptor | fst::kIDeterministic | fst::kODeterministic | fst::kWeighted | fst::kCyclic),
                               type_("pylm"),
  ...
  virtual Weight Final(StateId sid) const override { /* cerr << "PylmFst::Final("<<sid<<") == " << (pylm_->GetState(sid).wid == EOS_ID ? 1 : 0) << endl; */ return pylm_->GetState(sid).wid == EOS_ID ? Weight::One() : Weight::Zero(); };
  virtual size_t NumInputEpsilons(StateId sid) const override { return 0; }
  virtual size_t NumOutputEpsilons(StateId sid) const override { return 0; }
  virtual uint64 Properties(uint64 mask, bool test) const override { /* cerr << "PylmFst::Properties() == " << properties_ << endl; */ return properties_ & mask; };
  ...
  const vector<A> * GetArcs(StateId stateId) const {
    /* cerr << "PylmFst::GetArcs("<<stateId<<")" << endl; */
