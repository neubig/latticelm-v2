...
template<class A>
float SampGen(const fst::Fst<A> & ifst, fst::MutableFst<A> & ofst, unsigned nbest, float anneal) { 
  typedef fst::Fst<A> F;
  typedef typename F::Weight W;
  typedef typename A::StateId S;

  fst::StateIterator< fst::Fst<A> > siter(ifst);

  // sanity check
  if(ifst.Start() == -1 || siter.Done())
  ...
  assert(stateWeights.size() > ifst.Start());
...
template
float SampGen<fst::LogArc>(const fst::Fst<fst::LogArc> & ifst, fst::MutableFst<fst::LogArc> & ofst, unsigned nbest, float anneal);
